  ...
  <main>
    <section id="profile-page">
      <div id="intro-profile">
        <h1 id="title-profile">Supprimer votre profil:</h1>
        <p id="text-profile">
          Êtes-vous sûr de vouloir supprimer votre compte ? Merci de confirmer
          votre nom et prénom:
        </p>
      </div>
      <form action="" method="post" id="form-profile">
        <label id="name-label" for="name"><i class="gg-profile"></i>Nom:</label>
        <input id="name-input" type="text" v-model="name" autofocus required />

        <label id="firstname-label" for="id"
          ><i class="gg-mail-forward"></i>Prénom:
        </label>
        <input id="firstname-input" type="text" v-model="firstname" required />
      </form>

      <!-- Boutton delete profile -->
      <ButtonDelete
        v-if="userId === idProfile || isAdmin === true"
        title="Bouton pour supprimer son profil"
        :action="action"
        @deleteButton="deleteProfile(this.$route.params.id)"
      >
      </ButtonDelete>
      <p id="succes">{{ userSucces }}</p>
      <p id="error">{{ error }}</p>
    </section>
  </main>
  <Footer />
...
export default {
  ...
  created() {
    ...
    fetch(
      ...
      .then((response) => {
        //Si erreur d'auth
        if (response.error || response.user.id != this.$route.params.id) {
          return this.$router.push({ path: "/" }); //On redirige vers le login
        }
    ...
    deleteProfile(userId) {
      ...
      fetch("https//localhost:3000/groupomania/profile/" + userId, {
        ...
        .then(async (result_) => {
          ...
          if (!user.error) {
            //Si pas d'erreur
            this.userSucces = "User supprimé !";
            return this.$router.push({ path: "/signup" }); //On redirige vers le feed
          }
